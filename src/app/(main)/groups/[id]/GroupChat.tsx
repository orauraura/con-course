...
export default function GroupChat({
  ...
  groupId: string
  userId: string
  initialMessages: GroupMessageWithProfile[]
}) {
  const [messages, setMessages] = useState<GroupMessageWithProfile[]>(initialMessages)
      ...
      .on(
        ...
        async (payload) => {
          const { data } = await supabase
            ...
            .single()
          if (data) setMessages((prev) => [...prev, data as GroupMessageWithProfile])
        }
      )
      .subscribe()
          ...
          return (
            <div key={msg.id} className={`flex gap-2 ${isMine ? 'flex-row-reverse' : ''}`}>
              ...
              <div className={`max-w-xs lg:max-w-md ${isMine ? 'items-end' : 'items-start'} flex flex-col`}>
                {!isMine && (
                  <span className="text-xs text-gray-500 mb-1">{msg.profiles?.display_name}</span>
                )}
                {msg.content && (
                  ...
                  <div className="mt-1">
                    {isImage ? (
                      ...
                      <a
                        ...
                        className="flex items-center gap-2 text-sm text-[#00b4aa] hover:underline bg-white px-3 py-2 rounded-lg shadow-sm"
                      >
                        <Paperclip size={14} />
                        {msg.file_name}
                      </a>
